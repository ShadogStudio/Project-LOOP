...
* 공개 Git은 플레이스홀더로 실행 가능한 베이스 (기획 확정 후)
...
3. Play 한다.
4. **WASD / 방향키**로 이동한다.
5. 보라 포털 → 던전, 노란 구체 → 런 루트, 빨간 적 → 전투, 초록 포털 → 마을 복귀(예치).
6. 공격: **마우스 좌클릭 / Space / J**
7. 마을 주황 상점 위: **1** Vitality(+Max HP), **2** Power(+Damage)

## 공개 저장소 vs 로컬 빌드

| 구분 | Git | 설명 |
|------|-----|------|
| `Assets/ProjectLOOP` | 포함 | 자체 코드·씬·플레이스홀더 (구현은 기획 후) |
